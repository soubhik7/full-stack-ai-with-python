...
> **55 course scripts, each paired with a beginner-to-advanced Jupyter notebook** — line-by-line explanations, prerequisites, Microsoft **AI-102** exam tips, and real alternative approaches for every file.

📍 **Track your progress:** open [`00_PROGRESS_TRACKER.ipynb`](00_PROGRESS_TRACKER.ipynb) at this chapter's root — one live checklist of **every concept notebook** across all 8 sections. Study in any order; any concept notebook you've run (and saved) is auto-detected and marked ✅ on the next tracker refresh, persisted to a git-ignored `concept_progress.json`, with `mark_done()`/`mark_undone()` manual overrides for concepts studied via the `.py` scripts. Logic lives in [`progress_tracker.py`](progress_tracker.py) — it re-scans the sections on every run, so newly added concepts show up automatically.
...
📝 **Exam prep:** [`EXAM_NOTES.md`](EXAM_NOTES.md) is a full AI-103 quick-revision guide: all 151 notebook exam tips condensed and reorganized under the official 5-domain AI-103 blueprint, plus every syllabus topic the course doesn't cover (Foundry IQ, Microsoft Agent Framework, A2A protocol, M365/Work IQ agent publishing, Voice Live, Sora 2 video generation, Language/Speech MCP servers, monitoring/security, responsible multimodal AI), with recap tables, mnemonics, and 55 flash facts. [`EXAM_PRACTICE_QUESTIONS.md`](EXAM_PRACTICE_QUESTIONS.md) adds 65 exam-style scenario questions (blueprint-weighted, with answers + explanations) for self-testing, and [`EXAM_PRACTICE_QUESTIONS_AGENTS_DEEPDIVE.md`](EXAM_PRACTICE_QUESTIONS_AGENTS_DEEPDIVE.md) adds 32 more (Q66–Q97) focused on Foundry Agent Service depth — tool orchestration/workflows, identity & deployment governance, safety/compliance controls, and observability. [`EXAM_CASE_STUDIES.md`](EXAM_CASE_STUDIES.md) adds 3 full case studies (20 questions) in the real exam's case-study format — a Company Overview / Existing Environment / Requirements scenario followed by mixed multiple-choice, multi-select, Yes/No, and ordered-steps questions that span multiple domains in one scenario, the way the real exam's case-study section does.
...
| `04_agent_frameworks/` | 7 | Agent frameworks on Azure — LangChain, LangChain + OpenTelemetry tracing, LangGraph, an MCP-backed Azure Function, plus the C# demo Function App MCP server consumed two ways: registered as a Foundry project connection (`connect_foundry_mcp`) and called inline from the Responses API's hosted `mcp` tool (`agent_with_functionapp_mcp`) |
...
| `06_language_and_speech/` | 13 | Azure AI Language (NER, PII, sentiment, language detection, translation) + Azure AI Speech (speech-to-text, text-to-speech, real-time & translated speech) — includes both LLM-prompted and dedicated-service approaches to the same tasks. Concept notes: [`CLU_Evaluation_Metrics.md`](06_language_and_speech/CLU_Evaluation_Metrics.md) (Precision/Recall/F1 for Conversational Language Understanding, why not BLEU) |
| `07_content_understanding/` | 3 | Document/content understanding agent — invoice analysis, image analysis, content agent. Concept notes: [`Vision_Service_Selection.md`](07_content_understanding/Vision_Service_Selection.md) (Spatial Analysis vs. Face Detection vs. Image Analysis vs. OCR, and why Content Understanding supersedes them on AI-103) |
